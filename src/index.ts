import { ConfigOptions, GoogleButtonViewOptions, InvokeTapdata } from "./interface";
...
export class GoogleWebSignIn {
    ...
    constructor(data: ConfigOptions) { this.uploadScriptAndInitialize(data) }
     ...
     */
    private uploadScriptAndInitialize(config: ConfigOptions): Promise<unknown> {
        try {
            if (window !== undefined) {
     ...
     * @returns - Promise<unknown>
     */
    invokeOneTap(data: InvokeTapdata): Promise<unknown> {
        try {
            if (this.isScriptLoaded) {
                if (this.google) {
                    this.google.accounts.id.prompt((notification: any) => data.notificationCallback && data.notificationCallback(notification));
                    return Promise.resolve();
                } else {
